# ADR-032 — Resolución autoritativa del tipo de cambio
...
- [ADR-003](ADR-003-money-representation.md): el importe original es el
...
- [ADR-015](ADR-015-frozen-rate-physical-representation.md): el tipo congelado
  es `(rate_coefficient bigint > 0, rate_scale smallint 0..12)`.
- [ADR-013](ADR-013-persisted-vs-derived.md) §6: qué se congela por conversión,
  y la tabla de correcciones —heredar frente a nueva resolución—.
- [ADR-008](ADR-008-exact-data-boundary.md) §4: el coeficiente cruza como texto
  y la escala como entero acotado.
- [ADR-028](ADR-028-offline-command-queue-and-optimistic-projection.md) §11 y
  §14: la taxonomía de respuestas de la cola y el conflicto monetario.

Lo que quedó **expresamente abierto**:
[ADR-009](ADR-009-authoritative-write-boundary.md) §8 enumera como no decidido
...
describirlo como resuelto _«sería inventarlo»_. ADR-013 §6 deja fuera lo mismo.
...
- `effective_date` es `date` y `effective_time` es `time` sin zona: un **reloj
  de pared local** ([ADR-020](ADR-020-version-content-and-time.md) §3).
...
4. **No se introducen manualmente** tipo, escala, fuente ni ningún valor
   equivalente. Las correcciones siguen ADR-013 y un tipo congelado no se
   modifica.
...
Todo lo demás de ADR-003, ADR-008, ADR-013, ADR-015, ADR-028 y ADR-029 sigue
vigente sin cambios.

### 2. La fecha efectiva, y lo que no la determina

**La fecha efectiva es `operation_version.effective_date`**: la fecha de
calendario que la persona declara, local y sin zona (ADR-020 §3).
...
la de la fuente.** La fecha efectiva no tiene zona (ADR-020 §3), así que el
...
trazable de Nomey (ADR-003 §3), fuera de F11.
...
(ADR-013 §6): el del Grupo y, si su base difiere, el Modo Personal del pagador.
...
- **Cobertura de la definición.** Una definición monetaria está cubierta por una
  ...
  (ADR-003 §3, ADR-004). El EUR es el pivote del BCE.
...
> recomendación de moneda por Region de ADR-019 §7 es un asunto distinto y no
> cambia.
...
1. Replay e idempotencia, antes que nada (ADR-011 §13).
...
**disponibilidad del dato**, y ADR-028 §11 ya incluye los 5xx en su fila
...
Además, es la fila conservadora que ADR-028 §11 prescribe: no afirma que la
...
conservar el código deja intactos ADR-028 §14, ADR-029 y el cliente vigente. Si
...
scale)` de ADR-015, con **escala canónica 12**, para que un mismo tipo tenga una
...
**Por qué no contradice el «único redondeo» de ADR-003 §5.** Ese redondeo es el
...
consumen **tanto** `src/domain/` **como** los checks SQL (ADR-002 §7, ADR-009
§1).
...
Un tipo en caché en el cliente **sólo sirve para previsualizar** (ADR-003 §4), y
nunca se envía.

**Corregir sigue la tabla de ADR-013 §6**, sin cambios:
...
- Si la nueva resolución no es posible, la corrección **recibe el resultado de
  §6** y no se aplica. Las correcciones no se encolan (ADR-028 §4): el resultado
  lo recibe directamente quien corrige.

ADR-003 §4 admite corregir _«el propio tipo»_ como dato. **En F11, ningún camino
...
una operación **explícita y trazable** (ADR-003 §3) que queda fuera de F11. Esto
**restringe los caminos disponibles** y no contradice ADR-003.
...
correspondencia de códigos (ADR-003 §4, ADR-013 §6, invariante 22 de
`data-model.md`).
...
- **La fecha de referencia de la publicación usada, la fuente y el método** se
  ...
  alterar las columnas de `core.frozen_conversion`** (ADR-015). Dónde y cómo, lo
...
en conflicto una operación capturada bajo la base anterior (ADR-028 §14). Con la
...
- **No se exige en correcciones.** La base es inmutable en cuanto existe un
  efecto (ADR-002 §8, FK compuesta), y una corrección presupone que existe, así
...
- **Si la base cambia EUR → USD → EUR antes de que se vacíe la cola**, la
  ...
  vigente y no se reinterpreta nada. Es una lectura deliberada de ADR-003 §7,
...
acepta por la misma razón que en ADR-027: **no hay producción**.
...
- **Cambiar la base de un Grupo**, que ADR-003 §7 permite a su creador, produce
...
| Vectores de derivación y conversión, consumidos por el dominio y por SQL con UUID fijos (ADR-004 §4)                                                    | F11.B |
| Medición por HTTP de los códigos y estados de §6                                                                                                        | F11.B |
| Cómo espera la cola una entrada _todavía no disponible_, y si merece una clase propia (sucesor de ADR-028 §11)                                          | F11.C |
...
| Estadísticas por categoría sobre la **magnitud económica convertida**, preservando el invariante de ADR-026 §6                                          | F11.C |
| Presentación: original como principal y convertido como secundario, atribución al BCE, previsualización de correcciones (ADR-013 §6)                    | F11.C |
| Mostrar la derivación al revisar un conflicto (ADR-003 §7), hoy vacía por ADR-029 §3                                                                    | F11.C |
...
- una revisión del proveedor cambiaría el tipo según el momento de escribir,
  contra §2;
- va contra el espíritu de ADR-003 §3, que pone la metadata monetaria bajo
  control de Nomey.
...
ADR-003 (alternativa I): metería en un hecho contable un dato decidido por un
cliente no confiable.

**Deducir la moneda extranjera de `currency_definition_id ≠ base vigente`.**
**Descartada:** convertiría en silencio la operación de ADR-028 §14.
...
ADR-015 y una migración para ganar fidelidad sobre un dato que en origen sólo
...
- **Los seis resultados se distinguen** por código y estado, y encajan en la
  taxonomía de ADR-028 §11 y en `response.ts` **sin tocar el cliente vigente**.
...
- **Espera breve en zonas adelantadas respecto a Fráncfort.** Una operación con
  ...
  efectos y la cola la reintenta con el backoff de ADR-028 §12. Medido: el worker
...
- **Hay supuestos vigentes que dejan de cumplirse por sí solos**, y F11.C debe
  preservarlos:
  - ADR-025 §3 da por hecho que la línea vigente y la tachada «hablan la misma
    unidad», pero una corrección puede cambiar la moneda original;
  - la comprobación de ADR-026 §6 (suma de categorías = `expense_total`) exige
...
- **«Cambio de moneda base con historia».** Las Consecuencias de
  [ADR-019](ADR-019-personal-provisioning.md) y `docs/PROJECT_STATE.md` lo sitúan
  en F11. Sin embargo, ADR-002 §8 y el invariante 12 fijan la base **inmutable
  ...
  ADR-002 §8. **Este ADR no lo decide.**
