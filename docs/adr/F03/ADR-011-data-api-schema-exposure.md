# F03/ADR-011 — Exposición definitiva de schemas de la Data API

- **Estado:** Aceptado
- **Fecha:** 2026-08-25
- **Identificador anterior:** ADR-014 (numeración única, anterior a la organización por fases del 2026-09-14)

## Contexto

[F03/ADR-002](../F03/ADR-002-schema-topology.md) fijó la topología —`core` para la
...
[F03/ADR-003](../F03/ADR-003-privilege-model.md) lo mantuvo fuera de su alcance por la
...
Basta con retirar cualquiera de los tres factores. F03/ADR-003 §7 ataca el segundo
...
F03/ADR-002 §4 dejó dicho que un objeto técnico futuro —una extensión, un artefacto
...
| `core`           | **No**                   | Persistencia interna (F03/ADR-002, F03/ADR-003 §6) |
| `sec`            | **No**                   | Helpers de seguridad (F03/ADR-002, F03/ADR-003 §6) |
...
distintos** y F03/ADR-003 §6 ya insiste en que ninguno sustituye al otro: estar en
...
Este ADR **resuelve el punto heredado de F03/ADR-002 §4 y nada más**. No decide:

- si `graphql_public` permanece a largo plazo;
- si algún objeto técnico futuro puede vivir en `public` — F03/ADR-002 §4 sigue
  vigente: se juzga cuando exista;
- ningún grant, ninguna política, ninguna tabla, ninguna columna.

**No contradice ni reemplaza a F03/ADR-002 ni a F03/ADR-003**: rellena un hueco que
...
saneamiento de F03/ADR-003 §7, que hay que acordarse de aplicar objeto a objeto.
...
**Descartada** por el mismo criterio con el que F03/ADR-003 §5 eligió
...
contradiría F03/ADR-002 §4, que deliberadamente **no** prohibió el uso técnico de
`public`.
...
F03/ADR-002 dejó abierta. Puede plantearse por separado.

**D · Mover la superficie a `public` y renunciar a `api`.**
**Descartada** por F03/ADR-002, que ya la evaluó y la rechazó como alternativa A de
...
- **El test de exposición que exige F03/ADR-003 §6 se amplía sin esfuerzo**: pasa de
  comprobar que `core` y `sec` no están, a comprobar la lista completa.
- **Coherencia con F03/ADR-002 §4**, que permite objetos técnicos en `public`
...
- **Depurar con Studio es menos directo**, que ya era una contrapartida asumida
  en F03/ADR-002: Studio mira `public` por defecto.
