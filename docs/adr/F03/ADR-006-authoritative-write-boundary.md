# F03/ADR-006 — Frontera autoritativa de escritura

- **Estado:** Aceptado
- **Fecha:** 2026-08-24
- **Identificador anterior:** ADR-009 (numeración única, anterior a la organización por fases del 2026-09-14)

## Contexto

[F01/ADR-001](../F01/ADR-001-accounting-model.md) §7 decidió lo esencial y no se rediscute:
...
- **[F03/ADR-003](../F03/ADR-003-privilege-model.md)** — privilegio mínimo explícito, y las
  lecturas del cliente atraviesan vistas `security_invoker` con la RLS como
  autoridad por fila.
- **[F03/ADR-004](../F03/ADR-004-membership-rls.md)** — la RLS resuelve la pertenencia con
  un helper reducido, y la expulsión surte efecto de inmediato.
- **[F03/ADR-005](../F03/ADR-005-exact-data-boundary.md)** §3 — los valores exactos entran
...
compartidos, no compartiendo código.** Es exactamente lo que F01/ADR-001 §7 previó.
...
- permite **observar el tipo JSON original** antes de convertir, que es lo que
  F03/ADR-005 §3 exige y un parámetro `text` no puede dar **[medido, E14]**;
...
- mantiene **junta la intención** que la idempotencia debe comparar
  ([F03/ADR-007](../F03/ADR-007-client-operation-idempotency.md));
...
roles cliente siguen sin `USAGE` sobre `sec`** (F03/ADR-004 §3).
...
**Decidido** (F02/ADR-001 §4, invariantes 9 y 27; `data-model.md` §10):
...
**No decidido**, y F02/ADR-001 lo deja expresamente fuera de alcance: que exista un
...
duplicación del cálculo**, que F01/ADR-001 aceptó como coste. **Descartada** por dos
...
cumplir F03/ADR-005 §3**. Se pierde ergonomía a cambio de una garantía que sin ella
no existe.
...
- El tipo JSON original es observable, de modo que F03/ADR-005 §3 pasa de obligación
  a mecanismo.
...
La idempotencia la fija [F03/ADR-007](../F03/ADR-007-client-operation-idempotency.md).
