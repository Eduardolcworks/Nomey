# F03/ADR-001 — Identidad física de la definición monetaria

- **Estado:** Aceptado
- **Fecha:** 2026-08-23
- **Identificador anterior:** ADR-004 (numeración única, anterior a la organización por fases del 2026-09-14)

## Contexto

[F02/ADR-001](../F02/ADR-001-money-representation.md) §3 ya decidió lo esencial: cada
...
**Lo único pendiente era su representación física en PostgreSQL**, y F02/ADR-001 lo
...
- F02/ADR-001 §3 sigue gobernando qué es una definición monetaria y cuándo dos
...
**Descartada por F02/ADR-001**, que es su alternativa C: en los tres casos reales
...
Cumple la letra de F02/ADR-001 —la identidad no es solo el código— pero **invita a
...
silencioso. Es exactamente el modo de fallo contra el que existe F02/ADR-001.
**Descartada.**

**E · Clave compuesta `(code, version)`.**

**Descartada**: mete el código ISO dentro de la clave foránea de cada fila con
dinero, que es precisamente lo que F02/ADR-001 §3 impide que sea la identidad. Es la
...
- **El schema donde vive el catálogo**, que fija
  [F03/ADR-002](../F03/ADR-002-schema-topology.md).
