# F03/ADR-012 — Representación física del tipo de cambio congelado

- **Estado:** Aceptado
- **Fecha:** 2026-08-25
- **Identificador anterior:** ADR-015 (numeración única, anterior a la organización por fases del 2026-09-14)
...
[F02/ADR-001](../F02/ADR-001-money-representation.md) §4 define el tipo de cambio
...
[F03/ADR-010](../F03/ADR-010-persisted-vs-derived.md) §6, posterior y específico de esta
...
[F03/ADR-005](../F03/ADR-005-exact-data-boundary.md) §4 fija la frontera de transporte:
...
Y `src/domain/money/exchange-rate.ts`, que F01/ADR-001 §7 obliga a que el servidor
...
**Esta decisión supersede exclusivamente la prescripción de F02/ADR-001 §4 de
almacenar el tipo como un valor `NUMERIC`.** Nada más de F02/ADR-001 se ve afectado,
y en particular **se mantienen intactas sus cuatro garantías**, que esta forma
cumple igual o mejor:

| Garantía de F02/ADR-001 §4                   | Cómo la cumple `(coefficient, scale)`                                 |
| -------------------------------------------- | --------------------------------------------------------------------- |
...
| **Acotado**                                  | `rate_scale between 0 and 12`, la cota que F02/ADR-001 §4 exige fijar |
...
**Sigue vigente sin cambios** todo lo demás de F02/ADR-001 §4: el tipo corresponde a
...
el propio tipo sea el dato corregido. Y sigue vigente F03/ADR-010 §6: **se congela el
valor, no una referencia**, y la procedencia **no es autoritativa**.

## Alternativas consideradas

**A · `NUMERIC(p, s)` con el valor decimal**, según la letra de F02/ADR-001 §4.
...
1. **Obligaría a convertir en la frontera, en los dos sentidos.** F03/ADR-005 §4
   ...
   y esa conversión es **exactamente el sitio donde F02/ADR-001 no quiere que pase
...
3. **`NaN` seguiría siendo representable.** El propio F02/ADR-001 §4 dedica un
...
cantidad, exactamente lo que F03/ADR-010 §5 y §6 rechazan al negarse a duplicar la
...
**C · Enmendar F02/ADR-001.** **Imposible por norma**: `docs/adr/README.md` declara
...
- **La escala es un dato declarado**, no una propiedad inferida de cómo el motor
  guardó un decimal.
- La cota que F02/ADR-001 §4 exigía declarar **queda declarada**, y es comprobable
  en catálogo.
...
- **Queda una divergencia documental viva.** F02/ADR-001 §4 seguirá diciendo
...
- **El proveedor, la fuente y el algoritmo de selección del tipo**, que F02/ADR-001
  §4 deja expresamente fuera y este ADR no toca.
- **La representación de la procedencia**, que F03/ADR-010 §6 declara opcional y no
