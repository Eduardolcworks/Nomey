# F03/ADR-010 — Persistido frente a derivado, reparto contextual y proyección canónica

- **Estado:** Aceptado
- **Fecha:** 2026-08-24
- **Identificador anterior:** ADR-013 (numeración única, anterior a la organización por fases del 2026-09-14)

## Contexto

[F03/ADR-008](../F03/ADR-008-operation-version-model.md) fijó el modelo físico de
...
El traspaso no era una formalidad. F03/ADR-008 §3 lo dejó escrito como el riesgo
...
Además, [F03/ADR-009](../F03/ADR-009-participant-identity.md) cerró la identidad de los
participantes sin resolver dónde viven la intención declarada de un reparto y su
resultado resuelto, y [F03/ADR-006](../F03/ADR-006-authoritative-write-boundary.md) §7
...
Los hechos son inmutables y corregir es versionar (F01/ADR-001 §6) · saldos y
...
varias conversiones derivadas (F02/ADR-001 §1) · el tipo de cambio queda congelado y
corresponde a la fecha efectiva (F02/ADR-001 §4, invariante 27) · la RLS de `core`
es la autoridad por fila a través de vistas `security_invoker` (F03/ADR-003 §5,
F03/ADR-004 §1) · el writer es no propietario y `NOBYPASSRLS`, y **sigue sometido a
RLS** (F03/ADR-006 §5-§6, medido en E16) · los efectos referencian siempre al
participante (F03/ADR-009 §3) · el participante es **contextual por ámbito** y los
de ámbitos distintos no se correlacionan (F03/ADR-009 §1).
...
y vive en `core.client_command` (F03/ADR-007, F03/ADR-008 §5).
...
transferencia o de la financiación de un Modo Pareja, y F01/ADR-001 §10 sustituye la
...
monetaria**, porque F02/ADR-001 §1 los declara el valor principal de la interfaz y
...
distinción que F02/ADR-001 preserva.

### 4. Vigencia

> **`current_version_id` es estado autoritativo persistido.** F03/ADR-008 §1 decide
...
significativa la restricción de F03/ADR-008 §14, que reduce la superficie mutable
del modelo a ese único puntero. Con la lectura contraria, cualquier `INSERT` en
la tabla de versiones alteraría qué cuenta.

> Una **anulación o revocación** futura —que F03/ADR-008 deja expresamente fuera de
...
F03/ADR-009 §1 fija que el participante es **contextual** y que los de ámbitos
...
la **moneda base del ámbito** de la cabecera, que F01/ADR-001 §8 y F02/ADR-001 §7 hacen
...
> el cierre**, con su bilateralidad (F01/ADR-001 §9, invariante 17) y sus reglas
...
**No se decide aquí**, y F02/ADR-001 lo deja expresamente fuera de alcance: el
...
creación, por el mismo motivo que F03/ADR-008 §9 adoptó `command_contract_version`:
...
**Tres dimensiones independientes**, tal como las enumeran F01/ADR-001 §1 y
`data-model.md` §1:
...
- las **funciones autoritativas de escritura** seguirán siendo PL/pgSQL —
  necesitan control de flujo y `RAISE` (F03/ADR-006 §9)— y quedan cubiertas por
...
| Relación            | Predicado                                                                   |
| ------------------- | --------------------------------------------------------------------------- |
| `effect`            | **membresía del ámbito**, mediante el helper de F03/ADR-004 §2              |
...
cadena —el motivo por el que F03/ADR-004 §2 lo eligió—, y los efectos no referencian
la operación.
...
> efectos. Es lo que F03/ADR-008 pretende al decir que el historial es consultable
...
efectos (F01/ADR-001 §7).

El writer conserva sus atributos de F03/ADR-006 §5 —`NOLOGIN`, no propietario,
...
rol cliente (F03/ADR-008 §15, medido en E17).
...
políticas RLS no son diferibles**: el recurso que F03/ADR-008 §4 usa para el puntero
...
**No se crea:** ninguna **tabla de visibilidad derivada** —F03/ADR-004 §6 la rechazó
...
> F01/ADR-001 §10 permite deliberadamente que una operación produzca efectos sobre
...
V2 **exige leer V1**: el siguiente `version_no` se calcula (F03/ADR-008 §12), el FX
...
de sentido la restricción de F03/ADR-008 §14.
...
exactamente lo que F03/ADR-004 §6 y su alternativa D rechazaron—; **no puede
...
- **Los efectos de versiones antiguas ocupan espacio para siempre.** Es el precio
  del historial, ya asumido por F03/ADR-008.
...
- **La segunda barrera es más débil en la escritura que en la lectura**, y lo es
  por una decisión de producto: F01/ADR-001 §10 permite efectos sobre el ámbito de
...
El modo de fallo dominante que F03/ADR-008 §3 dejó señalado —sumar historia y
...
- **F03/ADR-008 §2** delegó la forma física de los datos autoritativos de la versión:
  §3 y §5 la fijan, sacando el reparto de la versión.
- **F03/ADR-008 §3** exigió una proyección canónica: §9 la fija.
- **F03/ADR-008 §16** y **F03/ADR-006 §7** dejaron abierto el mecanismo de lock de la
  deuda: §11 lo fija.
- **F03/ADR-006 §6** delegó la forma concreta de las políticas del writer: §10 fija
...
ámbito con saldo pendiente, que F03/ADR-009 §12 deja sin representación física · el
...
compartidos de corrección que F03/ADR-008 ya reclamaba —V1 = 60, V2 = 75, V1
