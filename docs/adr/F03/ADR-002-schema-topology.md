# F03/ADR-002 — Topología de schemas y frontera de la Data API

- **Estado:** Aceptado
- **Fecha:** 2026-08-23
- **Identificador anterior:** ADR-005 (numeración única, anterior a la organización por fases del 2026-09-14)
...
- **[F01/ADR-001](../F01/ADR-001-accounting-model.md) §7** — el cliente móvil no es la
  ...
  **los roles cliente no tienen permisos de escritura sobre operaciones ni
  efectos**.
- **[F02/ADR-001](../F02/ADR-001-money-representation.md) §6** — ningún importe monetario ni
...
- **F01/ADR-001 §7** deja de depender de que los grants estén bien puestos sobre cada
  tabla: si la tabla no está en un schema expuesto, no hay ruta de escritura
  directa que revisar.
- **F02/ADR-001 §6** deja de depender de la disciplina de quien escriba cada
...
- **El mecanismo concreto de la frontera textual** que hace cumplir F02/ADR-001 §6
...
con importes puede exponerse sin violar F02/ADR-001 §6, la superficie de vistas y
...
- **Facilita garantizar F01/ADR-001 y F02/ADR-001 estructuralmente** en vez de por
