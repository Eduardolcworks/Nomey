# F03/ADR-005 — Frontera de datos exactos

- **Estado:** Aceptado
- **Fecha:** 2026-08-24
- **Identificador anterior:** ADR-008 (numeración única, anterior a la organización por fases del 2026-09-14)

## Contexto

[F02/ADR-001](../F02/ADR-001-money-representation.md) §6 fijó la garantía: **ningún importe
...
resultado normativo en F02/ADR-001 §10—, que además demostró que **lo determinante
...
[F03/ADR-003](../F03/ADR-003-privilege-model.md) §5 ya fijó que las lecturas del cliente
...
> evidencia en decisión sería exactamente el error que F03/ADR-002 §4 advierte.
...
Es compatible con F02/ADR-001, que en su §4 define conceptualmente el tipo de cambio
...
`CurrencyDefinitionId` es un `UUID` ([F03/ADR-001](../F03/ADR-001-currency-definition-identity.md)),
...
—F02/ADR-001 lo dice y no cambia—, pero dejan de ser una trampa.
...
camino de F03/ADR-003. **Descartada como solución completa** porque no dice nada de
...
aprobado** en F03/ADR-003 §5 —vistas `security_invoker` con la RLS de `core` como
autoridad— y volvería a poner la autorización dentro de cada función, que es
justo el modo de fallo abierto que F03/ADR-003 descartó tras medirlo en E13.
...
- **La garantía de escritura no queda cerrada aquí.** F03/ADR-005 fija la
...
**F03/ADR-005 fija únicamente el contrato exacto de transporte.**
