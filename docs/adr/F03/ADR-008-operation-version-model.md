# F03/ADR-008 — Modelo físico de operaciones, versiones y comandos cliente

- **Estado:** Aceptado
- **Fecha:** 2026-08-24
- **Identificador anterior:** ADR-011 (numeración única, anterior a la organización por fases del 2026-09-14)

## Contexto

[F01/ADR-001](../F01/ADR-001-accounting-model.md) §6 y
...
[F03/ADR-006](../F03/ADR-006-authoritative-write-boundary.md) fijó la frontera de escritura
—funciones por clase, payload `jsonb`, writer de mínimo privilegio sometido a
RLS, transacción única— y [F03/ADR-007](../F03/ADR-007-client-operation-idempotency.md) la
...
es lo que F03/ADR-007 prohíbe expresamente.
...
Son **tres comandos distintos**, cada uno sujeto a F03/ADR-007, y **A es una sola
...
que F03/ADR-007 §3 reserva para otra cosa.
...
generar una intención nueva. Es justo el duplicado que F03/ADR-007 existe para
impedir.
...
**Descartada por F01/ADR-001 §6**, que prohíbe mutar los hechos. No es una
...
**Descartada por desproporción**, la misma razón por la que F01/ADR-001 descartó el
...
originadas por recurrencias, importaciones o backend —que F03/ADR-007 deja
abiertas— tendrían esas columnas a `NULL`: campos que solo significan algo para
algunas filas, el patrón que F01/ADR-001 rechazó al descartar la transacción única
con discriminador.
