# F03/ADR-013 — Atribución económica de efectos a un usuario

- **Estado:** Aceptado
- **Fecha:** 2026-08-26
- **Identificador anterior:** ADR-016 (numeración única, anterior a la organización por fases del 2026-09-14)

## Contexto

[F03/ADR-009](../F03/ADR-009-participant-identity.md) cierra su documento delegando
expresamente:
...
**[F03/ADR-010](../F03/ADR-010-persisted-vs-derived.md) es D11, y no la recogió.**
...
siempre al participante (F03/ADR-009 §3) · la dimensión de saldo **no tiene ningún
campo de identidad propio** (F03/ADR-010 §8) · el participante económico es
**legítimamente nulo** en el Modo Personal (F03/ADR-010 §8) · el saldo del Modo
...
(F03/ADR-009 §6) más `debtor <> creditor` hacen imposible ser las dos partes.
...
como máximo un Modo Personal (F01/ADR-001 §2). Sin clave foránea a `auth.users`,
...
`core.membership` significa **autorización actual** — así lo define F03/ADR-009 §4 y
...
anterior**, contra F03/ADR-009 §12, que fija que el participante histórico nunca
...
original**»— y F03/ADR-009 §7. No se usan los instantes de registro:
...
invocar funciones de `sec` por nombre**, porque F03/ADR-004 §3 le niega `USAGE` sobre
...
`effective_date` · `dimension` · `amount` como **texto** (F03/ADR-005 §1).
...
> **Esto NO resuelve el «acceso residual» de F03/ADR-009 §12**, que sigue abierto:
...
F03/ADR-009 §4 define la membresía como otra cosa.
...
—E20 ya lo había medido— y F03/ADR-010 §8 permite expresamente que una fila lleve
