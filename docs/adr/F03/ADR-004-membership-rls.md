# F03/ADR-004 — Comprobación de membresía y estrategia de RLS

- **Estado:** Aceptado
- **Fecha:** 2026-08-24
- **Identificador anterior:** ADR-007 (numeración única, anterior a la organización por fases del 2026-09-14)

## Contexto

[F01/ADR-001](../F01/ADR-001-accounting-model.md) dejó pendiente, en otro ADR, el
...
[F03/ADR-002](../F03/ADR-002-schema-topology.md) §4 la volvió a dejar abierta, y
[F03/ADR-003](../F03/ADR-003-privilege-model.md) §5 fijó que las lecturas del cliente
...
las vistas de `api` declaradas `security_invoker` (F03/ADR-003 §5).
...
Se mantiene además `REVOKE EXECUTE ... FROM PUBLIC` (F03/ADR-003 §4) y `sec` fuera
de los schemas expuestos y de los search paths de las superficies API (F03/ADR-003
§6).
