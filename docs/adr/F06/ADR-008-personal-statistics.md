# F06/ADR-008 — Estadísticas agregadas del Modo Personal

- **Estado:** Aceptado
- **Fecha:** 2026-08-31
- **Identificador anterior:** ADR-026 (numeración única, anterior a la organización por fases del 2026-09-14)
...
[F06/ADR-007](../F06/ADR-007-personal-read-surface.md) §1 fijó **cuatro superficies de
...
**Lo que no se reabre:** la unidad de lectura es la operación (F06/ADR-007 §2) ·
`api.personal_effect` conserva su propósito de atribución por dimensión
([F03/ADR-013](../F03/ADR-013-economic-attribution.md)) · el saldo se deriva sin caché
([F03/ADR-010](../F03/ADR-010-persisted-vs-derived.md) §1) · `core.current_effect` es la
proyección canónica y el límite de privilegio (F03/ADR-010 §9) · los importes cruzan
`api` como texto ([F03/ADR-005](../F03/ADR-005-exact-data-boundary.md) §1) · sólo `ingreso`
y `gasto` alimentan estadísticas ([F01/ADR-001](../F01/ADR-001-accounting-model.md) §4) ·
`effective_date` es el eje de agrupación por día, mes y año
([F06/ADR-002](../F06/ADR-002-version-content-and-time.md) §3).

## Decisión

### 1. Por qué las cuatro superficies de F06/ADR-007 no bastan
...
para pintar la home, que es justamente lo que F06/ADR-007 evita.
...
es derivada. **La tercera capa de F03/ADR-010 §1 —cachés— sigue vacía.**

`SECURITY INVOKER`, como `api.observed_balance` y por el mismo motivo: esta
lectura **no debe atravesar la RLS**. `BEGIN ATOMIC`, porque F03/ADR-010 §9 lo exige
...
admitidos de F01/ADR-001 §4 es aquí **estructural**, no una condición que alguien
pueda olvidar.
...
- **F06/ADR-007 §1 deja de ser cierto en su literalidad**: son cinco superficies, no
...
| **Paginar el intervalo entero y sumar**                | Descargar el historial para pintar la home, que es lo que F06/ADR-007 evita                      |
...
| **Añadir `category_id` a `api.personal_effect`**       | Su unidad es el efecto; la categoría es del movimiento (F06/ADR-002 §1). Mezclaría dos preguntas |
...
| **Materializar las estadísticas**                      | La caché que F03/ADR-010 §1 mantiene vacía, con todas sus preguntas de drift                     |
