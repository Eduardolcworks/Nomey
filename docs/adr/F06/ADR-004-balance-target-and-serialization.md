# F06/ADR-004 — Ajuste por saldo objetivo y serialización de la dimensión saldo

- **Estado:** Aceptado
- **Fecha:** 2026-08-29
- **Identificador anterior:** ADR-022 (numeración única, anterior a la organización por fases del 2026-09-14)
...
F01/ADR-001 §7 e invariante 20: **el cliente envía intención, no resultado**. Y por
medición, **E22/R1**:
...
de los efectos vigentes, sin caché en v1 ([F03/ADR-010](../F03/ADR-010-persisted-vs-derived.md) §1)
· una corrección crea versión nueva y no muta la anterior
([F03/ADR-008](../F03/ADR-008-operation-version-model.md)) · `core.current_effect` es la
proyección canónica (F03/ADR-010 §9) · el protocolo de serialización de F03/ADR-010 §11,
su orden ascendente y su criterio de pertenencia **por efectos y no por nombre**
· una función pública por clase de operación ([F03/ADR-006](../F03/ADR-006-authoritative-write-boundary.md) §1).
...
existente; **no se crea una segunda**: F03/ADR-006 §1 fija una por **clase de
...
concepto y categoría ([F06/ADR-002](../F06/ADR-002-version-content-and-time.md)): **no toda
...
carrera. Es literalmente la regla de F03/ADR-010 §11.
...
Por **efectos**, como manda F03/ADR-010 §11. Siete de las ocho clases producen
...
[F06/ADR-005](../F06/ADR-005-balance-observation.md) quien convierte toda escritura de saldo
...
que F03/ADR-010 §11 declara equivalente a no serializar nada.
...
**Sin concepto ni categoría**, como decidió F06/ADR-002: su línea de historial la
...
| **Función pública aparte para el objetivo**                       | Dos funciones para la clase `adjustment`, contra F03/ADR-006 §1                                                                       |
...
| **Materializar el saldo para tener fila que bloquear**            | Segunda fuente de verdad; F03/ADR-010 alternativa B ya la descartó, y el lock sobre `core.scope` responde el único argumento a favor  |
| **Advisory locks por ámbito**                                     | Mecanismo paralelo sin razón material. F03/ADR-010 §11 lo reserva como escalada **si una medición muestra contención**                |
