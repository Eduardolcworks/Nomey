# F06/ADR-006 — Anulación de una operación

- **Estado:** Aceptado
- **Fecha:** 2026-08-29
- **Identificador anterior:** ADR-024 (numeración única, anterior a la organización por fases del 2026-09-14)
...
**[F03/ADR-008](../F03/ADR-008-operation-version-model.md) lo dejó expresamente fuera**, y
...
Y por esa misma razón F03/ADR-008 §11 **no añadió** el
...
(F03/ADR-008) · `core.current_effect` es la proyección canónica
([F03/ADR-010](../F03/ADR-010-persisted-vs-derived.md) §9) · quién puede corregir es la
**membresía actual** del ámbito (`data-model.md` §7) · el protocolo de
serialización de F03/ADR-010 §11, extendido al saldo por
[F06/ADR-004](../F06/ADR-004-balance-target-and-serialization.md).
...
que prohíbe la guarda de catálogo de F03/ADR-010 §9. Ya ocurrió en F6.A con una
...
> Un helper `SECURITY DEFINER` —la salida que F03/ADR-004 §2 estableció para
> `sec.is_member`— también se probó y se descartó: habría tenido que leer
> `core.effect` directamente, contra la guarda de F03/ADR-010 §9, que **también
> saltó**.
...
- **Observación de saldo**: sí, aunque no tenga efectos propios. Los ámbitos
  ...
  auditoría** ([F06/ADR-005](../F06/ADR-005-balance-observation.md) §4).
...
**El `UNIQUE` de linaje que F03/ADR-008 §11 no añadió sigue sin añadirse.** Aquel ADR
...
pública por clase de operación» (F03/ADR-006 §1): esa regla existe porque cada clase
...
| **Efectos de importe cero**                       | Fabrica hechos que no ocurrieron para esquivar la RLS. F03/ADR-010 §8 conserva ceros **resueltos por indivisibilidad**, no inventados |
| **Sin discriminante, por ausencia de efectos**    | La vista de F6.D dependería de `core.effect` y la guarda de F03/ADR-010 §9 la rechaza                                                 |
