# F06/ADR-002 — Contenido no monetario y grano temporal de la versión

- **Estado:** Aceptado
- **Fecha:** 2026-08-28
- **Identificador anterior:** ADR-020 (numeración única, anterior a la organización por fases del 2026-09-14)
...
[F03/ADR-010](../F03/ADR-010-persisted-vs-derived.md) §3 enumera el contenido de
...
inmutables y corregir crea una versión nueva ([F03/ADR-008](../F03/ADR-008-operation-version-model.md))
· la clase vive solo en `core.operation` y no se duplica (F03/ADR-010 §3) ·
`operation_class` es un vocabulario **abierto** a propósito (F03/ADR-010 §2) · la
canonicalización la hace **solo** el servidor y «no degrada ni reformatea los
valores exactos» (F03/ADR-008 §8) · `core.participant_period` es de grano `date`
**porque su único consumidor es `effective_date`**
([F03/ADR-009](../F03/ADR-009-participant-identity.md) §7).
...
y normalizar caben en la primera cláusula de F03/ADR-008 §8 —«materializar los
defaults semánticos»— y no tocan ningún valor exacto de F02/ADR-001.
...
- **Que la familia de la categoría coincida con la clase real de la operación no
  ...
  relación que mapee clase de operación a familia contable, y **F03/ADR-010 §2 dejó
...
| ------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **Columnas anulables en `operation_version`**                             | Su obligación depende de una clase que la fila **no contiene**, y F03/ADR-010 §3 se niega a duplicarla. Quedaría una tabla que no puede explicar sus propios nulos |
| **Meter la clase en la versión** para poder poner un `CHECK` condicionado | Contradice F03/ADR-010 §3, y duplicar la clase es exactamente lo que ese ADR evita                                                                                 |
