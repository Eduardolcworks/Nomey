# F06/ADR-007 — Superficie de lectura del Modo Personal

- **Estado:** Aceptado
- **Fecha:** 2026-08-30
- **Identificador anterior:** ADR-025 (numeración única, anterior a la organización por fases del 2026-09-14)
...
[F03/ADR-013](../F03/ADR-013-economic-attribution.md). El producto necesita otra cosa: una
...
- **Las anuladas se excluyen por `version_kind`**, no por ausencia de efectos:
  ...
  [F03/ADR-010](../F03/ADR-010-persisted-vs-derived.md) §9, como ya ocurrió en F6.A.
- **`observed_balance_after` es ilustrativo**, y una vista de `api` que dependa
  de `core.balance_observation` hace fallar el check de
  [F06/ADR-005](../F06/ADR-005-balance-observation.md) §3.4.
...
en v1 (F03/ADR-010 §1) · `core.current_effect` es la proyección canónica y el
**límite de privilegio** (F03/ADR-010 §9) · los importes cruzan `api` como **texto**
([F03/ADR-005](../F03/ADR-005-exact-data-boundary.md) §1) · corregir versiona y no muta
([F03/ADR-008](../F03/ADR-008-operation-version-model.md)) · anular es una versión sin
efectos, terminal, y su trazabilidad es **interna**
([F06/ADR-006](../F06/ADR-006-annulment.md)) · el ajuste declara `delta` **o**
`target_balance` ([F06/ADR-004](../F06/ADR-004-balance-target-and-serialization.md) §1) ·
concepto, categoría y hora viven donde el hecho existe
([F06/ADR-002](../F06/ADR-002-version-content-and-time.md), [F06/ADR-003](../F06/ADR-003-category-catalogue.md)).
...
separación es exactamente el invariante que F06/ADR-005 protege.
...
de F03/ADR-013 y, con ella, la superficie desde la que se derivan las **estadísticas**
que sólo `ingreso` y `gasto` alimentan (F01/ADR-001 §4). Ampliarla a lista de
...
| `original_amount` | El importe **declarado** de la versión (F03/ADR-010 §3). Un gasto lo declara en + |

No es redundancia. F01/ADR-001 §2 existe para que el movimiento de caja y el hecho
...
eso sea seguro lo garantiza `OPERATION_CLASS_MISMATCH` (F06/ADR-002): todas las
versiones de una operación son de la misma clase.

**No se fabrica el signo con un `case` sobre la clase.** Sería aritmética
paralela a la que ya hizo el escritor, que es lo que F06/ADR-005 rechazó para el
saldo.
...
- **El discriminante de «Editado» es `previous_version_id`**, no `version_no - 1`.
  F03/ADR-008 §11 reservó a la frontera el invariante «el predecesor es la versión
...
- **La RLS que lo hace posible ya existía**: `operation_version_client_select`
  ...
  `core.effect` filtra por ámbito y no por vigencia. Es lo que F03/ADR-010 §10 quería
...
F06/ADR-006 fija que la trazabilidad de una anulada «solo es alcanzable por vía
...
su dueño — lo que F06/ADR-006 ya falsificó en su §D6.
...
El check de F06/ADR-005 cuenta las vistas de `api` que dependen de
...
Una función consigue lo mismo **sin tocarlo**: F03/ADR-010 §9 ya estableció que las
...
observación es del instante en que **su versión** se escribió (F06/ADR-005 §5), y la
...
> de **todos** los efectos vigentes (F03/ADR-010 §1); truncarlo a las clases
...
Sin concepto ni categoría, como decidió F06/ADR-002: salen **nulos**, no vacíos ni
sintéticos. Las dos formas se distinguen por `target_balance`, tal como F06/ADR-004
§1 las define:
...
partida doble** — la guarda de F03/ADR-010 §9 prohíbe que dependan de él tanto una
...
| **`version_no - 1` como predecesor**                  | F03/ADR-008 §11 **no** hizo estructural que el predecesor sea la versión anterior                                           |
...
| **Vista de `api` para la observación**                | El check de F06/ADR-005 §3.4 la rechaza. Y ampliarlo a «exactamente una vista» debilita el invariante literal               |
...
| **Historial de las anuladas en `api`**                | F06/ADR-006 las saca de la superficie normal; publicarlas como historial las devuelve por la puerta de atrás                |
...
| **Denormalizar el nombre de la categoría**            | Rompería en silencio «renombrar alcanza al histórico» de F06/ADR-003. Se publica `category_id` y lo resuelve `api.category` |
