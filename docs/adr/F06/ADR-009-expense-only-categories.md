# F06/ADR-009 — La categoría es del gasto, y el icono es una clave semántica

- **Estado:** Aceptado
- **Fecha:** 2026-09-01
- **Identificador anterior:** ADR-027 (numeración única, anterior a la organización por fases del 2026-09-14)
- **Supersede parcialmente:** [F06/ADR-003](../F06/ADR-003-category-catalogue.md), §1, §2,
  §3, §4, §5 y la primera consecuencia aceptada. El resto de F06/ADR-003 sigue
...
F06/ADR-003 modeló las categorías con **dos familias** —`expense` e `income`— y una
...
- **El icono se guardaba como nombre de SF Symbol.** `fork.knife`, `figure.run`.
  F06/ADR-003 lo aceptó explícitamente como consecuencia («acopla el dato a la
...
en la misma migración ([F03/ADR-003](../F03/ADR-003-privilege-model.md)) · que el actor
salga del JWT · que `core.client_command` sea la unidad de idempotencia
([F03/ADR-008](../F03/ADR-008-operation-version-model.md) §5) · que las escrituras no
contables pasen por `nomey_provisioner`
([F06/ADR-001](../F06/ADR-001-personal-provisioning.md)).
...
Es la misma forma que F06/ADR-002 §2 ya fijó para el resto del contenido de la
...
([F03/ADR-006](../F03/ADR-006-authoritative-write-boundary.md)).
...
**baja lógica**, nunca `DELETE`, que es F06/ADR-003 §7 sin modificar. Suministros y
...
Las identidades no se regeneran. Siguen siendo los UUID v5 de F06/ADR-003 §3, con
...
exactamente lo contrario del acoplamiento que F06/ADR-003 aceptó: **la base dice qué
...
versión entera**, no un delta ([F03/ADR-008](../F03/ADR-008-operation-version-model.md) §4),
así que la categoría vuelve a ser obligatoria en cada una. Omitirla es
`PAYLOAD_INVALID · 400`.

La autorización al asignar es la de F06/ADR-003 §5 **menos la comprobación de
...
- **Las categorías personalizadas en ámbitos compartidos siguen sin resolver**, y
  esta decisión no lo prejuzga. Es la consecuencia abierta de F06/ADR-003, intacta:
  **F9 deberá decidirlo**.
...
| **`DELETE` de las cinco retiradas**                        | Irreversible, y arrastraría el histórico que las usa. La baja lógica ya resolvía esto en F06/ADR-003 §7                          |
