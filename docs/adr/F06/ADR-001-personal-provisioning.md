# F06/ADR-001 — Provisioning del Modo Personal y siembra del catálogo monetario

- **Estado:** Aceptado
- **Fecha:** 2026-08-28
- **Identificador anterior:** ADR-019 (numeración única, anterior a la organización por fases del 2026-09-14)
...
([F03/ADR-013](../F03/ADR-013-economic-attribution.md), [F03/ADR-004](../F03/ADR-004-membership-rls.md))
· la identidad monetaria es el `UUID` y no el código ISO
([F03/ADR-001](../F03/ADR-001-currency-definition-identity.md)) · las monedas admitidas son
...
una API externa** ([F02/ADR-001](../F02/ADR-001-money-representation.md) §3) · el cliente
envía intención y el servidor deriva ([F01/ADR-001](../F01/ADR-001-accounting-model.md) §7)
· una función pública por clase de **operación**
([F03/ADR-006](../F03/ADR-006-authoritative-write-boundary.md) §1) · la moneda base de un
...
**F03/ADR-006 §1 no las alcanza**: ninguna es una clase de operación contable. No
...
llevan participante **legítimamente nulo** (F03/ADR-010 §8) y `api.personal_effect`
...
lo que [F03/ADR-009](../F03/ADR-009-participant-identity.md) §1 evita. Si F10 lo necesita,
...
- **`nomey_writer`** escribe contabilidad **debajo** de la RLS. Su única
  ...
  protocolo de deuda de F03/ADR-010 §11, y no puede escribirla. Ensancharlo
...
origen cliente** ([F03/ADR-008](../F03/ADR-008-operation-version-model.md) §5). Ninguna de
...
un check lo comprueba— y existe porque **F03/ADR-001 dice expresamente que dos
...
F02/ADR-001 §3 designa. **JPY y CLP están a propósito**: son las que hacen fallar
...
importe; F03/ADR-005 §1 alcanza a los valores monetarios.
...
`core.effect`, y **la guarda de catálogo de F03/ADR-010 §9 la rechazó**: la única
...
| **Edge Function**                                          | Segunda superficie de escritura fuera de la frontera de F03/ADR-006                                      |
...
| **Resolver la moneda por código en el cambio**             | La identidad es el UUID; el código puede repetirse (F03/ADR-001)                                         |
