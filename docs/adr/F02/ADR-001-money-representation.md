# F02/ADR-001 — Representación exacta del dinero

- **Estado:** Aceptado
- **Fecha:** 2026-08-19
- **Identificador anterior:** ADR-003 (numeración única, anterior a la organización por fases del 2026-09-14)
...
[F01/ADR-001](../F01/ADR-001-accounting-model.md) fijó el modelo contable y dejó
...
- **Error de Nomey en la metadata**: corregir la tabla no cambia en silencio el
  ...
  por el versionado de F01/ADR-001 §6.
...
servidor** al recibir la operación, coherente con F01/ADR-001 §7.
...
tipo queda **versionado, atribuido, en historial y notificado** según F01/ADR-001.
...
una reconciliación real de saldo, según F01/ADR-001 §3.

**Mayor resto.** El algoritmo de F01/ADR-001 §5 trabaja con **enteros exactos** y no
...
Es una **restricción que introduce este ADR**, no una lectura de F01/ADR-001:
F01/ADR-001 no enuncia que el total de un reparto sea no negativo, aunque tampoco
...
(F01/ADR-001 §8). Este ADR resuelve el intervalo anterior:
...
F01/ADR-001: ese invariante condiciona la notificación a que haya **efectos
...
> **Esto no contradice F01/ADR-001 §10** —«no hay confirmaciones, efectos pendientes
...
> que F01/ADR-001 descarta es la confirmación **de un tercero**, mientras que aquí
...
cliente no confiable, contra F01/ADR-001 §7.
...
- La aritmética es determinista y reproducible en cliente y servidor, requisito
  ya impuesto por F01/ADR-001 §7 para la previsualización sin conexión.
