# F07/ADR-001 — Cola de escritura sin conexión, durabilidad de la clave y proyección optimista

- **Estado:** Aceptado
- **Fecha:** 2026-09-03
- **Identificador anterior:** ADR-028 (numeración única, anterior a la organización por fases del 2026-09-14)
...
[F02/ADR-001](../F02/ADR-001-money-representation.md) §7 sin reinterpretar nada en
silencio.
...
- **La clave de idempotencia no es durable.** Vive en un
  ...
  respuesta»_. **[F03/ADR-007](../F03/ADR-007-client-operation-idempotency.md) §1 exige
...
[F03/ADR-010](../F03/ADR-010-persisted-vs-derived.md) §1. La entrada en ~5 segundos con
...
F03/ADR-007 §1 dejó escrita, usa la puerta que F03/ADR-007 §6 dejó abierta, y respeta
[F03/ADR-008](../F03/ADR-008-operation-version-model.md) §5 sin tocarla.
...
operación sin red, y es la prueba documental del conflicto de F02/ADR-001 §7.
...
> **Enmienda de 2026-09-06 · F9.** `group.create` es el tercer valor, y el
> primero que no es un movimiento personal. Se añade aquí y no en un ADR
> aparte porque el vocabulario **es** este ADR: un valor que viviera fuera
> sería exactamente el nombre de función libre que §3 prohíbe. Lo que no
> cambia es el mecanismo — sigue siendo un discriminante cerrado, sigue
> traduciéndose con un `switch` exhaustivo, y un valor desconocido sigue sin
> ejecutarse nunca.
...
> **Enmienda de 2026-09-06 · F9.** La cola enruta además **`group.create`**, y
> el runtime pasa a vivir en `features/shell` con el transporte inyectado por
> comando: una sola instancia, una sola persistencia y una sola barrera, que es
> lo que esta sección exige. **Correcciones, anulaciones y ajustes siguen
> fuera**, y siguen sin incorporarse por analogía.
>
> `group.create` cabe aquí y aquéllas no por la misma razón que las separaba:
> **no lleva CAS y no puede quedar obsoleta**. No corrige nada anterior, no
> tiene versión previa que comparar y su idempotencia es una clave, no un
> estado — [F09/ADR-002](../F09/ADR-002-client-provisioning-idempotency.md). Un reintento
> es un replay, nunca un `VERSION_CONFLICT`.
>
> **No es un hecho contable**, así que no toca `core.client_command` ni crea
> operación alguna. La cola transporta la intención; quién la hace idempotente
> es asunto de la frontera que la recibe.
...
- **`conflict`** — la operación es válida y lo que se movió fue la
  configuración monetaria bajo ella. F02/ADR-001 §7 exige revisión, no descarte, y
...
exactamente** ([F01/ADR-001](../F01/ADR-001-accounting-model.md) §7,
[F03/ADR-006](../F03/ADR-006-authoritative-write-boundary.md) §1) y cuya paridad afirman los
...
([F06/ADR-009](../F06/ADR-009-expense-only-categories.md))— así que no tiene sitio en
`domain/effects`.
...
- **`record_adjustment` declara siempre el Disponible CONFIRMADO**, nunca el
  ...
  protocolo de [F06/ADR-004](../F06/ADR-004-balance-target-and-serialization.md) hace que
...
- **Una petición en vuelo**, y **FIFO por actor**. El servidor ya serializa los
  ámbitos bajo un orden ascendente de locks (F06/ADR-004), así que el paralelismo
...
### 14. El conflicto monetario de F02/ADR-001 §7
...
> **Un límite honesto.** F02/ADR-001 §7 pide además enseñar la derivación bajo la
...
habituales: es una situación que **F02/ADR-001 §7 exige** tratar aparte. Ofrece
...
toca la tercera capa de F03/ADR-010 §1.
...
tercera capa que F03/ADR-010 §1 declara vacía en v1, y su introducción exige medición,
...
fallara**. Es el mismo razonamiento por el que F03/ADR-007 §3 prohíbe devolver el
...
- **F03/ADR-007 §1 se cumple de verdad.** La clave existe en disco antes del primer
...
| F02/ADR-001 §2, §3, §6                     | **Intactos.** Unidades mínimas enteras, definición monetaria con su identidad, y nada monetario cruza JSON como número                                  |
| F02/ADR-001 §7                             | **Se implementa** en §14, con su límite de FX dicho                                                                                                     |
| F03/ADR-007 §1, §2, §3                     | **Se cumple** §1 por fin; §2 y §3 siguen siendo del servidor y el cliente no canonicaliza ni compara nada                                               |
| F03/ADR-007 §6                             | **Se usa** la puerta que dejó abierta, en los términos que dejó escritos                                                                                |
| F03/ADR-008 §5                             | **Intacto.** La unidad de idempotencia sigue siendo el comando, y el `command_type` local es un espejo del suyo, no una fuente                          |
| F03/ADR-010 §1 · tercera capa vacía        | **Intacta.** F03/ADR-010 define caché como «se persistiría»; aquí no se persiste ningún agregado                                                        |
| F03/ADR-010 §9 · proyección canónica       | **Intacta.** La vigencia se sigue resolviendo en `core.current_effect` y no se reimplementa en el cliente                                               |
| F06/ADR-004                                | **Intacto y reforzado:** §10 impide declarar un `target_balance` sobre una cifra proyectada                                                             |
| F06/ADR-006                                | **Intacto.** La anulación sigue siendo terminal y no se encola (§4)                                                                                     |
