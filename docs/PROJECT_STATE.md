...
Actualizado el **2026-09-17**, al **abrir la Fase 12 y cerrar F12.A0** (los
cuatro ADR de la fase aceptados: username e identidad pública, transferencias
entre usuarios con dos voluntades, transferencia dentro de un grupo y solicitud
de pago por enlace; **sin implementación**). La **Fase 10** cerró el
2026-09-16 y la **Fase 9** el 2026-09-14. **F11.A** está cerrada (decisiones
de multimoneda, F11/ADR-001, sin implementación) y F11.B se desarrolla en
paralelo en otra rama.
...
|                         |                                                                                                                                                                                                                                                                                                                                                                                                               |
| ----------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Fases en curso**      | **Fase 8** (distribución interna: F8.A0 … F8.A5 cerrados, **F8.B** y **F8.C** pendientes) y **Fase 11** abierta (**F11.A** cerrada, contrato sin implementación; F11.B en preparación). **Fase 11** abierta: **F11.A** cerrada (contrato, sin implementación) **Fase 12** abierta el 2026-09-17: **F12.A0** cerrado (cuatro ADR aceptados), **F12.A … F12.D** pendientes, **ninguna implementación iniciada** |
...
| **Backend**             | Migrado y reconstruible desde cero, con CI verificándolo en cada PR. **52 migraciones**: las cuatro últimas dejan la identidad permanente (F10/ADR-002), el vínculo activo/histórico (F10/ADR-003), el punto de inicio del Personal tras el Invitado (F10/ADR-005) y los dos cierres de F10/ADR-004 (retirar a un origen fusionado rehusado; una fusión es de un salto por catálogo)                          |
...
**La Fase 12 está ABIERTA (2026-09-17) y sólo F12.A0 está cerrado: cuatro ADR
aceptados y ninguna línea de implementación.** Nada de lo que sigue existe
todavía en el código; es lo que las siguientes sesiones implementarán, y lo
que ninguna fase debe deducir por su cuenta
([`docs/adr/F12/`](adr/F12/README.md); roadmap, Fase 12):

- **El alcance original de F12 se reconcilió contra el repositorio.** `shares`
...
| **Transferencias entre usuarios, username, solicitud de pago** | **Decididas en F12.A0** (F12/ADR-001…004, 2026-09-17); **implementación no iniciada**: F12.A (identidad pública), F12.B (transferencias y solicitud), F12.C (cliente), F12.D (cierre)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
