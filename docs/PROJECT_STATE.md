...
Actualizado el **2026-09-24**, al **implementar F12.C3 con su contrato final** (la transferencia
de grupo en el cliente: el segundo modo del selector que la ventana del `+` ya tenía → elegir a quién,
un formulario con la moneda del grupo fijada e importe vacío, la
vista previa de lo que el importe hará con la deuda, y la guarda temprana de
moneda que evita crear una propuesta que nadie podría aceptar. `20261006120000`.
Implementada y **pendiente de validación manual y de mergear**). Antes, al
**implementar F12.E.C y F12.E.D** (enlace
...
catálogo FX, la ingesta con fijación diaria, el resolver en SQL y la conversión
en los dos writers personales (B1–B5). Convierten `personal_expense` y
`personal_income`, y ninguna otra clase.
...
|                         |                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| ----------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Fases en curso**      | **Fase 8** (distribución interna: F8.A0 … F8.A5 cerrados, **F8.B** y **F8.C** pendientes) y **Fase 11** abierta (**F11.A** cerrada; **F11.B**: B1–B5 integrados, convierten solo `personal_expense` y `personal_income`; **F11.C** integrado: lecturas, estadísticas, edición y cola de las operaciones en moneda extranjera; **F11.D** integrado: el gasto de grupo convierte, reparte después y persiste la cuota personal). **Fase 12** abierta el 2026-09-17: **F12.A0** y **F12.A** cerrados (identidad pública: backend, alta con hook y cliente con gate, Perfil y offline-first, 2026-09-19); **F12.B cerrado** (2026-09-20): B1 (propuestas y `internal_transfer` de dos voluntades, `20260926120000`), B2 (solicitudes de pago mediante enlace, `20260927120000`) y B3 (propuestas de grupo y `settlement_by_transfer` algebraica, `20260928120000`); después F12.C y F12.D |
...
propio (**F10**, cerrada el 2026-09-16: ver abajo; las cesiones y fusiones
que F9 dejó pendientes quedaron **fuera** por F10/ADR-004, y la revocación
del vínculo ajeno **prohibida**), las transferencias entre usuarios (que F12
decidió como propuesta + aceptación, nunca unilaterales) y la «liquidación
sólo deuda» del escenario 4.5, que sigue en el writer
(`record_debt_settlement`) sin superficie en la app. Trasladados
...
**La Fase 10 está CERRADA (abierta el 2026-09-14, cerrada el 2026-09-16);
A0 … C0 cerrados; B1 y B2 no existen.** El punto de entrada es
[`phase-10-handoff.md`](architecture/phase-10-handoff.md), con los trece
criterios demostrados uno por uno. Su alcance
...
instancia, **[F10/ADR-003](adr/F10/ADR-003-active-and-historical-link.md)
(Aceptado, 2026-09-15)**: el vínculo es **activo** o **histórico**, y
**[F10/ADR-004](adr/F10/ADR-004-identity-scope-closure.md) (Aceptado,
2026-09-16)**, que cierra el alcance: ninguna cesión ni fusión entra. Lo que
hay que saber antes de tocar identidad:
...
- **No hay cesiones ni fusiones nuevas** (F10/ADR-004): ni `identity_handover`
  ni cesión cuenta → cuenta (la identidad activa es fija mientras la cuenta
  participa), ni fusión de dos participantes con cuenta, ni fusión fantasma ↔
  fantasma (el modelo la sostendría —medido en B0 sobre la matriz de la
  apertura— pero no hay necesidad de producto; un duplicado sin cuenta se
  retira). Fantasma → cuenta ya lo resuelven reclamar (F09/ADR-004) y asociar
  (F09/ADR-009). **Las cadenas de fusión son un invariante prohibido**: un
  origen nunca es destino, un destino nunca es origen, resolución de un solo
  salto. Lo impone el **catálogo** (migración `20260920120000`, C0): un
  trigger de fila sobre `core.participant_merge`, definer, en insert y update,
  rehúsa con `PARTICIPANT_MERGED` a un origen que ya es destino, a un destino
  que ya es origen y cualquier reapuntado, escriba quien escriba; además
  `api.associate_participant` sigue siendo el único escritor en `api`. Y
  **retirar o dar por saldado a un origen fusionado se rehúsa** con
  `PARTICIPANT_MERGED` en `sec.retire_participant_core`, el núcleo que
  comparten las dos puertas (la regresión medida en B0, cerrada en C0;
  `merge-invariants.sql`).
...
CERRADOS (F12.A el 2026-09-19, F12.B el 2026-09-20); F12.C está EN CURSO
(C1 hecha el 2026-09-20; C2 rechazada por decisión de producto el
2026-09-20 — sin UI de solicitudes de pago, el backend B2 se conserva —;
C3, la transferencia de grupo en el cliente, está IMPLEMENTADA en rama y pendiente de validación manual y de mergear, y con un CONTRATO NUEVO ([F12/ADR-007](adr/F12/ADR-007-one-will-group-transfers.md), que supera el de producto de F12/ADR-003 y conserva su implementación dormida): la transferencia de grupo es una **declaración de UNA voluntad** —sin propuesta, sin aceptación, sin rechazo— y una **operación del libro del GRUPO**, clase propia `group_transfer`. El receptor autoritativo es el PARTICIPANTE, con las reglas de cualquier alta del grupo (del ámbito, elegible hoy, no retirado, no fusionado, no uno mismo) y NINGUNA más: **un participante sin cuenta recibe como cualquiera** — ni username, ni Modo Personal, ni amistad, ni FX de ningún Personal ajeno. La caja es **sólo la del emisor** (un efecto por el total, si tiene Personal) y NADA entra en el Personal de ningún receptor; el débito es caja, no consumo, así que no toca Gastos ni Ingresos. **Multi-destinatario ATÓMICO**: una intención, una operación, N efectos de `settlement` y N filas de `core.group_transfer_allocation`; si un receptor falla no se escribe ninguno. El **reparto lo hace el servidor** con `sec.allocate_by_largest_remainder` (pesos a uno) sobre el orden canónico `(created_at, id)`, el mismo que `api.group_transfer_candidates` entrega y que el cliente usa para su vista previa; por debajo de una unidad menor por destinatario, `TRANSFER_AMOUNT_TOO_SMALL`. **No se corrige y SÍ se anula** (F06/ADR-006), con la guarda de sobreliquidación recorriendo TODOS los pares y sólo para quien la registró. La UI es la ventana que el `+` ya abría, en el segundo modo de su selector —importe, concepto y lista con ticks desde el principio, nadie preseleccionado, multiselección—, una fila por operación en el histórico (`api.group_transfer_operation` + `api.group_transfer_allocation`) y `api.group_operation` sin tocar. **Sin pendientes: ni Notificaciones ni campana.** Todo B3 (`20260928120000`) se conserva y duerme, con su check y sus nueve carreras verdes, igual que B2. F12.E (Amigos) está ABIERTO desde el 2026-09-22: amistad
...
persistencia local). **Las notificaciones push de transferencias quedan
diferidas**: eventos previstos `proposal_received`, `proposal_accepted` y
`proposal_declined`, sin importe ni concepto en el payload; en iPhone exigen
Apple Developer, APNs y una build instalada (F8.B), y no hay
`expo-notifications` en el proyecto
...
- **F12.A1 — backend de identidad (`20260921120000`).** `core.account_identity`
  ...
  handle: reserva provisional · definitivo · retenido, derivado de marcas; índice
  único parcial: un handle vivo por cuenta), `core.account_handle_event` (diario
  insert-only: el historial que ADR-001 §2/§9 exige, con desalojo perezoso de
  reservas caducadas y retenciones vencidas), `core.reserved_handle` (25 exactos +
  4 prefijos, gemelo de `tests/vectors/username.json`) y
  `core.username_lookup_attempt` (quién y cuándo, nunca el texto). Comandos
  `api.reserve_username` · `claim_username` · `change_username` (cooldown 30 días
  con `details.available_at`; el anterior retenido 90; recuperarlo cuenta como
  cambio) · `set_public_name`; `api.resolve_username` (`found | not_found | self |
throttled`, 20 consultas / 10 min; sólo handle y nombre); vista propia
  `api.my_account_handle`; `sec.public_identity(uid)` (uid → identidad actual,
  §13). Todo del `nomey_provisioner`, nada de `postgres`, sin BYPASSRLS.
- **F12.A2 — alta y Auth (`20260924120000`).** El alta por correo **exige**
  `requested_username` y `display_name`: el hook `sec.before_user_created`
  (definer del provisioner, la **única** función de `sec` que ejecuta
  `supabase_auth_admin`) crea la identidad y **reserva 7 días** en la misma
  transacción de GoTrue, o rehúsa el alta entera (`USERNAME_REQUIRED · 400`,
  `USERNAME_INVALID · 400`, `USERNAME_RESERVED · 422`, `USERNAME_TAKEN · 409`;
  medido: GoTrue lo entrega como `{"code":N,"error_code":"unknown","msg":"CÓDIGO"}`).
  El hook **no reclama**. Anónimo y otros proveedores pasan sin username.
  Invitado → cuenta **reserva antes** de `updateUser` (`api.reserve_username`
  con la sesión anónima, mismo uid). El hook está activo en `config.toml`
  local; **un proyecto alojado debe activarlo** (runbook).
- **F12.A3 — cliente (sin SQL).** Ciclo de una sola RPC por sesión:
  `claim_username` — reserva viva → definitivo; definitivo → sin escribir; sin
  reserva o caducada → `USERNAME_REQUIRED`, y **sólo ese veredicto del
  servidor** abre el **gate** (nombre público precargado con el `display_name`
  de sesión + username; sin «Saltar»; `reserve_username` reclama en el acto) en
  lugar de las pestañas. Un **invitado nunca pregunta ni ve el gate**. Perfil
  enseña la identidad pública de `core` (nombre público + `@username`), edita
  el nombre **`api.set_public_name` primero y la metadata de Auth después** (si
  la segunda falla no se deshace la primera: se enseña lo de `core` y se avisa),
  y cambia el username con `api.change_username`; el lápiz respeta
  `can_change_at` y el cooldown se enseña con su fecha. Sin historial ni
  retenidos.
- **Offline first (F07/ADR-001), sin ambigüedad:** un fallo de red **no es**
  `USERNAME_REQUIRED`. Una cuenta normal entra y usa Nomey sin conexión; la
  última identidad **definitiva** confirmada por el servidor se respalda por
  cuenta en el almacén offline de F7 (`account-identity`), pero **el respaldo
  no es autoridad**: el servidor lo sobreescribe siempre (incluido con
  `required`) y nunca abre el gate. Sin respaldo y sin respuesta, un
  **watchdog de 10 s** deja entrar como `unavailable`; al **volver al primer
  plano** —el único `AppState` listener, el mismo seam que despierta la cola—
  el ciclo vuelve a preguntar **sólo** si sigue sin confirmación; sin polling.
  ...
  escenario; no bloquea el cierre.
- **El alcance original de F12 se reconcilió contra el repositorio** (F12.A0): `shares`
  y `exact_amounts` están hechos (F3/F9; falta validarlos en dispositivo, en
  el cierre); las correcciones con elegibilidad ya las fijan F09/ADR-003 §5 y
  F09/ADR-008; el «acceso residual» y las «bajas con saldo» **no existen**
  (F09/ADR-007 C5/C6/C8, F10/ADR-003); el participante histórico está hecho.
  La «transferencia ordenada unilateral» que describía el roadmap **no
  existe**: toda transferencia entre usuarios exige **dos voluntades**.
- **Username** ([F12/ADR-001](adr/F12/ADR-001-username-public-account-identity.md)):
  atributo público, único y resoluble de la cuenta, en una relación propia de
  `core` (nunca en la metadata de Auth); `^[a-z](_?[a-z0-9])*$`, 3–20, ASCII,
  25 reservados exactos y 4 prefijos; **reservado en la misma transacción del
  alta** por el hook `before_user_created` (función `SECURITY DEFINER` de
  `sec`, único privilegio de `supabase_auth_admin`; sin RPC para `anon`, sin
  Edge Function, sin trigger en `auth.users`); reserva provisional de 7 días
  que caduca si `claimed_at` es nulo **sin mirar Auth**; `claim` en el primer
  ciclo autenticado; cambio con cooldown de 30 días y retención de 90;
  `public_name` en `core`; resolución **exacta** frenada a 20 consultas / 10
  min; y en todo histórico **`uid → identidad pública actual`**, nunca un
  username persistido ni resuelto hacia atrás. Sin contactos, teléfono, SMS ni
  búsqueda por correo.
- **Transferencia entre usuarios** ([F12/ADR-002](adr/F12/ADR-002-two-will-user-transfers.md)):
  ...
  (intención no contable: importe, moneda y destinatario fijos; sin efectos;
  7 días; `pending → accepted | declined | cancelled | expired`); sólo al
  aceptar nace una `internal_transfer` (−N / +N), **irreversible**: una sola
  versión, ni corrección ni anulación; devolver es otra transferencia.
  `operation.created_by` es **quien materializa** (el receptor); emisor y
  receptor viven en **partes por versión**, nunca se derivan de `created_by`.
  Invariante 14 precisado: «originar» = autorizar la salida propia por
  propuesta. 3 pendientes por pareja, 10 propuestas/hora por emisor. Supera el
  contrato de F3 de `record_internal_transfer`. **Implementado en F12.B1
  (`20260926120000`, 2026-09-19):** `core.transfer_proposal` y
  `core.transfer_part`; `api.create_transfer_proposal` (destinatario por
  `handle`, resuelto una vez en servidor; `not_found` es un estado, no un
  error, para que el freno compartido del resolver apunte), `cancel_` y
  `decline_transfer_proposal`; `api.record_internal_transfer` recreada con el
  payload `{proposal_id}` y una sola versión (`TRANSFER_NOT_EDITABLE`,
  `OPERATION_NOT_ANNULLABLE` en `annul_operation` y en `sec.persist_version`);
  presupuesto exacto bajo cerrojo transaccional por emisor; vistas
  `api.my_transfer_proposals` y `api.my_transfers`. Precisiones en
  [`docs/adr/F12/README.md`](adr/F12/README.md).
- **Transferencia dentro de un grupo** ([F12/ADR-003](adr/F12/ADR-003-group-transfers.md)):
  `Grupo → + → Transferencia → participante` (activo y con cuenta, del mismo
  grupo); al aceptar nace una `settlement_by_transfer`: `transfer ∓N` en los
  Personales y `settlement −N` sobre el par **por el importe completo,
  cruzando cero** (78 debidos + 80 transferidos → el acreedor debe 2). Supera
  `data-model.md` §3 **sólo** para esta clase con dos voluntades; `group_payment`
...
- **Solicitud de pago** ([F12/ADR-004](adr/F12/ADR-004-payment-request-links.md)):
  ...
  la solicitud, **un solo uso**, 7 días, sin `declined`, cancelable por el
  creador, previsualización frenada como la invitación, máximo 20 pendientes
  propias; pagarla materializa una `internal_transfer` del pagador al
  solicitante y `paid` es para siempre. Sin grupo, sin deuda. **Implementado
  en F12.B2 (`20260927120000`, 2026-09-20):** `core.payment_request` (sólo el
  sha256 del token) y `core.payment_request_attempt`; `api.create_payment_request`
  (el token viaja una vez; el replay devuelve `token: null`; tope exacto de
  20 pendientes bajo cerrojo por creador), `api.cancel_payment_request`,
  `api.preview_payment_request` (estados, nunca excepciones; sólo los
  `invalid` cuentan, 20 / 10 min); `api.record_internal_transfer` con dos
  orígenes XOR (`proposal_id` o `payment_request_token`): importe, moneda y
  concepto salen sólo de la solicitud, `paid_by` = pagador = `created_by` =
  dueño de `transfer_part.from_scope_id`; vistas `api.my_payment_requests` y
  `api.my_transfers` (con `payment_request_id`). El enlace compartible que
  incorpora el token **no se construye: por decisión de producto
  (2026-09-20) el cliente no expone ninguna UI de solicitud de pago** — sólo
  transferencias por `@username` — y el backend B2 queda íntegro, sin
  reabrir el ADR. Precisiones en
  [`docs/adr/F12/README.md`](adr/F12/README.md).
- **Fuera de F12:** contactos, teléfono, SMS/OTP, búsqueda por correo, banca
  real, Open Banking, tarjeta, bote común, préstamos, adelantos, pagos
  programados, transferencias múltiples, solicitudes dentro de un grupo,
  transferencias a cuentas inexistentes, FX nuevo, Unicode en el username,
  Modo Pareja (F13, **diferido previsiblemente hasta después del lanzamiento**
  por decisión de producto, 2026-09-17, sin salir del modelo ni del roadmap).
- **Con F11 en paralelo:** F12 no recrea ninguna función que F11 recree
  (`record_personal_*`, `record_group_expense`, `api.personal_operation`,
  estadísticas) y publica sus filas por superficies propias hasta que F11.C
  cierre; las transferencias no convierten (F11/ADR-001 §4).

**F11.A está CERRADA, y la conversión existe ya para dos clases:** de F11.B
están integrados el catálogo FX, la ingesta y la fijación diaria del tipo, el
resolver y los dos writers personales, que convierten y congelan la conversión
con su procedencia. [F11/ADR-001](adr/F11/ADR-001-fx-rate-resolution.md) fija lo que F11.B y F11.C
implementarán; toda operación de cualquier otra clase en una moneda distinta de
la base sigue respondiendo `CURRENCY_CONVERSION_UNSUPPORTED · 422`. Lo que una fase futura no
...
**El punto de inicio del Modo Personal tras el Invitado lo decide la persona,
una sola vez ([F10/ADR-005](adr/F10/ADR-005-personal-start.md), Aceptado
2026-09-16, F10.C0).** El Personal creado bajo un JWT anónimo lleva la marca
`core.scope.provisioned_as_guest` (la escribe `ensure_personal_scope` del
claim `is_anonymous`; nunca vuelve a `false`). La primera vez que esa cuenta,
ya convertida, abre Personal **con historia de grupos**, Inicio pregunta
«¿Cómo quieres empezar tu Modo Personal?»: **incluir** (todo como siempre) o
**empezar desde cero**; **sin** historia, el cliente persiste `include`
automáticamente para que la pregunta no pueda aparecer días después. Un hecho
por ámbito, `core.personal_start`, insert-only, con un único escritor,
`api.start_personal_scope` (comando de provisioning; idempotente por clave y
por estado; `PERSONAL_START_DECIDED` ante una segunda decisión distinta;
`automatic` rehusado con `PERSONAL_START_DECISION_REQUIRED` si hay historia;
`PERSONAL_START_NOT_APPLICABLE` para una cuenta sin marca). **Empezar desde
cero es un corte, no un borrado ni un ajuste**: por `core.operation.created_at`
frente a `started_at` (hora de servidor; nunca `effective_date`), en UN
predicado, `sec.counts_in_personal`, que aplican `sec.derive_balance` y todas
las lecturas del Personal (`personal_balance`, `personal_effect`,
`personal_operation`, `my_shared_expense_shares`): las operaciones de origen
grupo anteriores salen del saldo, el historial, las estadísticas y las
cuotas; **nunca de Deudas** (`group_summary`) ni de `claimed_dimension`. Una
corrección o anulación posterior de una operación anterior sigue fuera; la
caja que incorpora un `associate_participant` posterior cuelga de la
operación original y sigue fuera; lo creado después entra. Evidencia:
`supabase/checks/personal-start.sql`, §15 de la frontera HTTP y
...
**`record_internal_transfer` es sólo aceptación** (F12/ADR-002, F12.B1): su
payload es `{client_operation_id, command_contract_version, proposal_id}` y
emisor, receptor, importe y moneda salen de la propuesta; una sola versión
`record`, ni corrección (`TRANSFER_NOT_EDITABLE`) ni anulación
(`OPERATION_NOT_ANNULLABLE`). Alrededor, tres comandos del provisioner —
`create_transfer_proposal` (`handle`, importe, moneda, concepto opcional;
devuelve `state` `pending` con `proposal_id` o `not_found`),
`cancel_transfer_proposal` y `decline_transfer_proposal` (`{proposal_id}`,
idempotentes por estado)— y dos vistas propias, `my_transfer_proposals` y
`my_transfers`. **Desde F12.B2 admite un segundo origen, exactamente uno
(XOR): `payment_request_token`**, con tres comandos más del provisioner —
`create_payment_request` (importe, moneda, concepto opcional; devuelve el
token UNA vez, el replay lo devuelve `null`), `cancel_payment_request`
(`{request_id}`) y `preview_payment_request(p_token)` (estados `ok | own |
...
```
ensure_personal_scope        crea ámbito + membresía, o devuelve el existente; publica provisioned_as_guest, start_mode y needs_start_decision
start_personal_scope         decide, una sola vez, cómo empieza el Personal de una cuenta de origen Invitado: include | fresh (F10/ADR-005)
set_personal_base_currency   cambia la moneda si el ámbito nunca tuvo un efecto
```

**Identidad pública de la cuenta (F12.A, F12/ADR-001), idempotentes por
estado:** `reserve_username` (anónimo sólo reserva; normal reserva y reclama),
`claim_username`, `change_username`, `set_public_name`; `resolve_username`
(estado, nunca un identificador interno); la vista propia `my_account_handle`.
Y `sec.before_user_created`, que GoTrue ejecuta en el alta: **no es superficie
`api`**, la única función de `sec` que `supabase_auth_admin` puede ejecutar.
...
| `api.personal_scope`             | El ámbito del actor, con su moneda base y su escala; y su punto de inicio (`provisioned_as_guest`, `start_mode`, `needs_start_decision`) |
...
14. **Una transferencia entre usuarios sólo existe con dos voluntades**
    (F12/ADR-002, F12/ADR-003, F12/ADR-004): quien envía la autoriza desde su
    Personal, quien recibe la acepta o la solicitó; una propuesta o solicitud
    pendiente no es contable. Materializada, tiene **una sola versión** y no
    se corrige ni se anula. `operation.created_by` es quien materializa;
    emisor y receptor viven en las partes.
15. **El username es un atributo público de la cuenta, nunca su identidad ni
    una credencial** (F12/ADR-001): ninguna operación persiste usernames ni
    nombres; todo histórico resuelve `uid → identidad pública actual`; un
    username nunca se resuelve hacia atrás.
...
personal difiera de la del grupo **no entra** en el total en lugar de sumarse
mal: convertirla exigiría resolver un tipo al leer. Detalle en el
...
| **Transferencias entre usuarios, solicitud de pago** | **Decididas en F12.A0** (F12/ADR-002…004, 2026-09-17); **username hecho** (F12.A, 2026-09-19); **F12.B cerrado** (2026-09-20): B1 (propuestas e `internal_transfer` de dos voluntades), B2 (solicitud de pago mediante enlace) y B3 (propuesta de grupo y `settlement_by_transfer` algebraica) hechos; **F12.C en curso**: C1 (transferencias Personal en el cliente) hecha; C2 (solicitud de pago y enlace) **rechazada por producto el 2026-09-20** — sin UI, backend B2 conservado —; C3 (grupo, `20261006120000` y `20261007120000`, **F12/ADR-007**: una voluntad, inmediata, por participante —fantasmas incluidos—, caja sólo del emisor, multi-destinatario atómico, anulable) **implementada en rama, pendiente de validación manual y de mergear**; falta F12.D (cierre). **Push de transferencias diferido** hasta Apple Developer/APNs/build instalada |
...
| **Lo que dejó la Fase 10**                      | [`architecture/phase-10-handoff.md`](architecture/phase-10-handoff.md) · [`adr/F10/`](adr/F10/README.md) · [F05/ADR-003](adr/F05/ADR-003-guest-session.md)    |
| **Continuar la Fase 12**                        | [`adr/F12/README.md`](adr/F12/README.md) · roadmap, Fase 12 · los cuatro ADR (username, transferencias, grupo, solicitud)                                     |
