...
/**
 * ¿PUEDE ESTA CATEGORÍA APARECER EN UN GASTO COMPARTIDO?
 *
 * Sólo las de sistema, y por una razón de privacidad concreta: una categoría
 * propia la escribió una persona, vive bajo su `owner_user_id` y la RLS de
 ...
 * equivalente a `owner_user_id is null`: o es de sistema con su clave, o es de
 * alguien con su texto, y no hay una tercera. Así que esto no infiere nada — lee
 * la misma condición que el servidor vuelve a exigir en
 * `sec.assert_shared_category_usable`, que es quien manda.
 *
 * **Es una limitación de este bloque, no una decisión de producto.** Las
 * categorías propias de un grupo son una decisión aplazada: cuando exista una
 * categoría cuyo dueño sea el ámbito y no una persona, esta condición cambia
 * aquí y en el servidor a la vez.
 */
export function isSharedCategory(category: CategoryRow): boolean {
  return category.message_key !== null && category.label === null;
}

/** El catálogo reducido a lo que un gasto compartido admite. */
export function sharedCategories(categories: readonly CategoryRow[]): readonly CategoryRow[] {
  return categories.filter(isSharedCategory);
}
