...
describe('importe monetario', () => {
  it('el mismo importe cambia de forma con el locale', () => {
    ...
    expect(plain(formatMoney(value, ES))).toBe('50,00 €');
  ...
  it('agrupa y separa según el locale, no según un formato inventado', () => {
    ...
    expect(plain(formatMoney(value, ES))).toBe('1.234.567,89 €');
  ...
  it('respeta el umbral de agrupación de cada locale', () => {
    ...
    expect(plain(formatMoney(money(50000n, JPY), ES))).toBe('50.000 JPY');
  ...
  it('coloca el signo donde lo pone el locale', () => {
    ...
    expect(plain(formatMoney(negative, ES))).toBe('-1234,56 €');
  ...
  it('puede forzar el signo, que es la señal no cromática de entra/sale', () => {
    ...
    expect(plain(formatMoney(money(-2500n, EUR), ES, { sign: 'always' }))).toBe('-25,00 €');
    expect(plain(formatMoney(money(0n, EUR), ES, { sign: 'always' }))).toBe('+0,00 €');
  });

  it('puede omitir la moneda para una columna que ya la declara', () => {
    expect(plain(formatMoney(money(123456n, EUR), ES, { currencyDisplay: 'none' }))).toBe(
      '1234,56',
    );
    expect(plain(formatMoney(money(1234567n, EUR), ES, { currencyDisplay: 'none' }))).toBe(
      '12.345,67',
    );
  });
  ...
  it('rellena cuando hay menos dígitos que escala', () => {
    expect(plain(formatMoney(money(7n, EUR), ES))).toBe('0,07 €');
    expect(plain(formatMoney(money(-7n, EUR), ES))).toBe('-0,07 €');
...
describe('exactitud por encima de 2^53', () => {
  ...
  it('conserva los dígitos también en es-ES y con escala 0', () => {
    expect(plain(formatMoney(moneyFromMinorString('98765432109876543210', JPY), ES))).toBe(
...
describe('número y porcentaje', () => {
  it('usa los separadores del locale', () => {
    expect(plain(formatNumber(1234567.89, ES))).toBe('1.234.567,89');
    expect(plain(formatNumber(1234567.89, EN))).toBe('1,234,567.89');
  });

  it('formatea un porcentaje desde el ratio', () => {
    expect(plain(formatPercent(0.65, ES))).toBe('65 %');
...
describe('fecha', () => {
  it('formatea una fecha de calendario según el locale', () => {
    expect(plain(formatDate('2026-08-27', ES, 'medium'))).toMatch(/27/);
    expect(plain(formatDate('2026-08-27', EN, 'medium'))).toMatch(/27/);
    expect(formatDate('2026-08-27', ES, 'long')).toContain('agosto');
  ...
  it('devuelve la entrada intacta si no es una fecha de calendario', () => {
    ...
    expect(formatDate('27/08/2026', ES)).toBe('27/08/2026');
    expect(formatDate('2026-08-27T10:00:00Z', ES)).toBe('2026-08-27T10:00:00Z');
  });

  it('da los nombres de mes del locale', () => {
    expect(monthNames(ES)[0]).toBe('enero');
    expect(monthNames(EN)[0]).toBe('January');
    expect(monthNames(ES)).toHaveLength(12);
  });
});
